927.ops.create.services () {
  ...
  if [[ ! -z ${_json} ]] && [[ $( ${cmd_echo} ${_json} | ${cmd_jq} '. | length' ) > 0 ]]; then
    ...
    for service in $( ${cmd_echo} ${_json} | ${cmd_jq} -c '.[] | select(.enable == true)' ); do 
      ...
      _service_description=$(           ${cmd_echo} ${service}  | ${cmd_jq} -r  '.name.string | if( . == null ) then "" else . end' )
      ...
      ${cmd_echo} Writing Service: ${_path}/${_alias}.cfg
      ${cmd_cat} << EOF.service > ${_path}/${_alias}.cfg
define service                         {
