...
_infra=$( /usr/lib64/naemon/plugins/927-livestatus_get.sh -h ${_tenancy} -r ${_resource} -s infra-compartments )
...
while IFS= read iac_item; do
  ...
  while IFS= read infra_item; do
    ...
    if [[ ${_infra_label} != ${_iac_label} ]] && [[ ${_infra_tenant} != ${_tenant} ]]; then
      ...
      _json=$( ${cmd_echo} ${_json} | ${cmd_jq} -c '.data.iac['${_count}'].infra.label   |=.+ "'"${_infra_label}"'"' )
      _json=$( ${cmd_echo} ${_json} | ${cmd_jq} -c '.data.iac['${_count}'].infra.tenant  |=.+ "'"${_infra_tenant}"'"' )
